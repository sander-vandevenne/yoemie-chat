...
app.get('/user/:id/:jsonp', function(req, res){
	
	setTimeout(function() { 
	
		console.log("test"+req.params.jsonp);
		
		//var people = { name: 'Dave', location: 'Atlanta' };
		
		Chat.findAll({ where: { view: 0 }, attributes: ['profile_from'] }).success(function(chats) {
			
			var messages = new Object();
			
			for (x in chats)
			{
				messages[x] = { profile_from: chats[x].profile_from };
			...
			//var peopleJSON = JSON.stringify(chats);
		
			//res.send(req.params.jsonp+"("+peopleJSON+")");
		})
